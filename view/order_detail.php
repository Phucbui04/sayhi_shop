        ...
        <td>
            <table class="container-lsdh" border="1">
                ...
                <tr>
                    ...
                    <th class="thead-item">Số lượng</th> 
                ...
                ?>
                            <tr >
                                ...
                                <td class="tbody-items"><?= $product_id ?></td>
                                <td class="tbody-items"><?= $item['amount'] ?></td>
                                <td class="tbody-items"><?= $item['quantity'] ?></td> 
                                <td class="tbody-items"><?= $item['creation_date'] ?></td>
                                <td class="tbody-items">
                                    <table >
                                        <form class="custom-form" action="" method="post" enctype="multipart/form-data">
                                            <tr>
                                                <input type="hidden" name="id" value="<?= $order['id'] ?>">
                                                <td><input type="submit" name="thaydoi" onclick='huyDonHang()' value="Hủy" style="background: transparent;border:none;padding: 0 15px;"></td>
                                            </tr>
                                            <tr>
                                                <td>
                                                    <select required name="status" id="status" style=" display:none">
...
<!-- <script>
  var daHuyDonHang = false;

  function huyDonHang() {
    var thayDoiTrangThai = document.getElementById("thaydoi");

    if (thayDoiTrangThai && !daHuyDonHang) {
      var check = confirm("Bạn muốn hủy đơn hàng?");
      if (check) {
        alert("Bạn đã chọn Tiếp tục.");
        daHuyDonHang = true;

        // Đặt thuộc tính disabled cho nút "Hủy"
        thayDoiTrangThai.disabled = true;
      } else {
