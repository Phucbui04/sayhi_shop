...
/* function updateOrderCancel($orderId) {
    $conn = connect();
    $sql = "UPDATE orders SET is_paid = 4 WHERE id = $orderId";
    $stmt = $conn->prepare($sql);
    ...
    try {
        ...
        if (isset($_SESSION['cart'])) {
            foreach ($_SESSION['cart'] as $id => $item) {
                ...
                $sqlDetail = "INSERT INTO orders_detail (`amount`, `order_id`, `product_id`, `id_user` ,`quantity`) 
                              VALUES (:amount, :order_id, :product_id, :id_user,:quantity)";
...
{
    $conn = connect();
    $sql = "SELECT o.id, o.amount, o.quantity, p.id as product_id, p.name, p.image
            FROM orders_detail as o
            INNER JOIN products as p ON o.product_id = p.id WHERE o.order_id = :order_id";
    $stmt = $conn->prepare($sql);
...
function all_order($user_id = 1) {
    ...
    $stmt = $conn->prepare($sql); 
    $stmt->bindParam(1, $user_id, PDO::PARAM_INT); //Lấy từ vị trí số 1 và của câu lệnh sql tức là id_user=? sau đó điền vào giá trị là $user_id
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $result;
}
...
{
    $conn = connect();
    $sql = "SELECT o.id, o.amount, o.id_user, p.id as product_id, p.name, o.creation_date ,o.quantity
            FROM orders_detail as o
